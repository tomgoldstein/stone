...
filename = ['DMD_reconstructions/pendcar/' type '_Nr_',num2str(Nr) ,'_Nd_' ,num2str(length(data)) ,'_Ndf_', num2str(dataPerFrame),'_shift_',num2str(shiftPerFrame),'_Nf_',num2str(Nf), '_mu_',num2str(mu), '_tvdims_',num2str(tvdims) ];
imdata = permute(frames,[1 2 4 3]);
imwrite(imdata,[filename '.gif'],'DelayTime',0,'LoopCount',inf);

imwrite(frames(:,:,end/2)/256,['DMD_reconstructions/pendcar/3dtv_sample_frame.png'],'mode','lossless');
